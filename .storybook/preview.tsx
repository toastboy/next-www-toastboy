...
const badgePng = new URL(
    '../src/tests/mocks/data/badge.png',
    import.meta.url,
).toString();
const flagPng = new URL(
    '../src/tests/mocks/data/flag.png',
    import.meta.url,
).toString();
const footballPng = new URL(
    '../src/tests/mocks/data/football.png',
    import.meta.url,
).toString();
const mugshotPng = new URL(
    '../src/tests/mocks/data/mugshot.png',
    import.meta.url,
).toString();
...
const mockAuthDecorator: Decorator = (Story, context) => {
    useEffect(() => {
        ...
        const { role = 'none', user } = (context.parameters?.mockAuth ??
            {}) as {
            role?: MockAuthState;
            user?: MockAuthUser;
        };
...
const authHandlers = [
    ...
    http.post('*/api/auth/sign-in/email', async ({ request }) => {
        ...
        const inferredRole = body.email?.toLowerCase().includes('admin')
            ? 'admin'
            : 'user';

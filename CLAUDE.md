...
- **Never** use Mantine's dot-notation compound components (`Table.Tr`, `Menu.Item`, `AppShell.Header`, `RichTextEditor.Bold`, etc.) — always import the standalone named subcomponent instead (`TableTr`, `MenuItem`, `AppShellHeader`, `BoldControl`, etc.) and use that directly in JSX. Reason: when a Server Component (no `'use client'`) accesses a dot-notation subcomponent of a `'use client'` package export, the property resolves to `undefined` at render time — "Element type is invalid: expected a string ... but got: undefined" — because Server Components only receive an opaque client reference for a `'use client'` export, not the real object, so its static properties aren't accessible. This is a confirmed, still-open limitation of the current Next.js/React Server Components architecture, not a bug in Mantine or this codebase (see [vercel/next.js#84961](https://github.com/vercel/next.js/issues/84961) and the root-cause explanation in [vercel/next.js#75192](https://github.com/vercel/next.js/issues/75192)). It bit `PlayerInfo`, `PlayerResults`, `PlayerPositions`, and `PlayerArse` in July 2026. The one exception is `RichTextEditor.Toolbar` (from `@mantine/tiptap`), which has no standalone export — leave it as dot notation, only in files that are already `'use client'`, with a comment explaining why.

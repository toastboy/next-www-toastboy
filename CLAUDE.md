...
- Submit buttons stay enabled and rely on `validateInputOnBlur` inline errors plus `form.onSubmit` blocking an invalid submit — don't gate submit on `disabled={!form.isValid()}`. Gating on something other than field validity (e.g. dirty-tracking, a selection count, an in-flight request) is fine and not covered by this rule

...
describe('MoneyForm', () => {
    ...
    it('renders unpaid player charges', () => {
        ...
        expect(
            screen.getByRole('heading', { name: 'Unpaid Player Charges' }),
        ).toBeInTheDocument();
    ...
    it('renders empty state when there are no debts', () => {
        ...
        expect(
            screen.queryByRole('button', { name: 'Paid' }),
    ...
    it('disables Pay button when all debt checkboxes are unchecked', async () => {
        ...
        expect(
            screen.getAllByRole('button', { name: 'Paid' })[0],
        ).toBeDisabled();
    });
    ...
    it('uses "player" in the loading notification when player name is null', async () => {
            ...
            players: [
                {
                    ...
                    debts: [
                        { gameDay: createMockGameDay({ id: 9 }), amount: 300 },
                    ],
                },
            ],
        });
        ...
        render(
            <Wrapper>
                <MoneyForm
                    playerDebts={nullNameDebts.players}
                    payDebt={payDebt}
                />
            </Wrapper>,
        );
        ...
        await waitFor(() => {
            const calls: { message?: string }[] =
                notificationsShowMock.mock.calls.map(
                    (c: unknown[]) => c[0] as { message?: string },
                );
            expect(
                calls.some((c) => c.message?.includes('for player across')),
            ).toBe(true);
        });
    });

...
describe('toHttpErrorResponse', () => {
    it('maps AppError subclasses to expected status codes', () => {
        expect(
            toHttpErrorResponse(new ValidationError('bad input')).status,
        ).toBe(400);
        expect(toHttpErrorResponse(new NotFoundError('missing')).status).toBe(
            404,
        );
        expect(toHttpErrorResponse(new AuthError('forbidden')).status).toBe(
            403,
        );
        expect(toHttpErrorResponse(new ConflictError('conflict')).status).toBe(
            409,
        );
        expect(
            toHttpErrorResponse(new ExternalServiceError('smtp down')).status,
        ).toBe(503);
    ...
    it('returns safe public messages for AppError and ZodError', () => {
        expect(toHttpErrorResponse(new AuthError()).message).toBe(
        ...
        expect(toHttpErrorResponse(zodError, 'Fallback').message).toBe(
            'Invalid request.',
        );
    });

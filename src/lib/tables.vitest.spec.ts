...
describe('TableTitle', () => {
    ...
    it('returns "Stalwart Standings" for the stalwart table', () => {
        expect(TableTitle(TableNameSchema.enum.stalwart)).toBe(
            'Stalwart Standings',
        );
    });
    ...
    it('returns a capitalised "<name> Table" for other tables', () => {
        ...
        expect(TableTitle(TableNameSchema.enum.averages)).toBe(
            'Averages Table',
        );
...
describe('QualifiedTableName', () => {
    it('combines "All-time" with the table title for year 0', () => {
        expect(QualifiedTableName(TableNameSchema.enum.points, 0)).toBe(
            'All-time Points Table',
        );
    });

    it('combines the year with the table title for non-zero years', () => {
        expect(QualifiedTableName(TableNameSchema.enum.points, 2025)).toBe(
            '2025 Points Table',
        );
        expect(QualifiedTableName(TableNameSchema.enum.speedy, 2024)).toBe(
            '2024 Captain Speedy',
        );
    });
});
...
describe('UnqualifiedTableName', () => {
    ...
    it('returns undefined for tables with no unqualified section', () => {
        expect(
            UnqualifiedTableName(TableNameSchema.enum.points),
        ).toBeUndefined();
        expect(
            UnqualifiedTableName(TableNameSchema.enum.stalwart),
        ).toBeUndefined();

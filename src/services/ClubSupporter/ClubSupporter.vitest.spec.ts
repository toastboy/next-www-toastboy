...
describe('clubSupporterService', () => {
    ...
    describe('create', () => {
        it('should create a ClubSupporter', async () => {
            (prisma.clubSupporter.create as Mock).mockResolvedValueOnce(
                defaultClubSupporter,
            );
            const result =
                await clubSupporterService.create(defaultClubSupporter);
            expect(result).toEqual(defaultClubSupporter);
        });

        it('should refuse to create a ClubSupporter with invalid data', async () => {
            await expect(
                clubSupporterService.create({
                    ...defaultClubSupporter,
                    playerId: -1,
                }),
            ).rejects.toThrow();
        ...
        it('should refuse to create a ClubSupporter that has the same player ID and club ID as an existing one', async () => {
            (prisma.clubSupporter.create as Mock).mockRejectedValueOnce(
                new Error('ClubSupporter already exists'),
            );
            await expect(
                clubSupporterService.create({
                    ...defaultClubSupporter,
    ...
    describe('upsert', () => {
        it('should create a ClubSupporter where the combination of player ID and club ID did not exist', async () => {
            (prisma.clubSupporter.upsert as Mock).mockResolvedValueOnce(
                defaultClubSupporter,
            );
    ...
    describe('upsertAll', () => {
        it('should upsert each club id for a player', async () => {
            const upsertSpy = vi
                .spyOn(clubSupporterService, 'upsert')
                .mockResolvedValue({
                    ...defaultClubSupporter,
                    playerId: 7,
                    clubId: 1,
    ...
    describe('delete', () => {
        it('should delete an existing ClubSupporter', async () => {
            (prisma.clubSupporter.delete as Mock).mockResolvedValueOnce(
                defaultClubSupporter,
            );
        ...
        it('should silently return when asked to delete a ClubSupporter that does not exist', async () => {
            ...
            (prisma.clubSupporter.delete as Mock).mockRejectedValueOnce(
                notFoundError,
            );
        ...
        it('should rethrow delete errors that are not P2025', async () => {
            ...
            await expect(clubSupporterService.delete(7, 16)).rejects.toThrow(
                'db exploded',
            );
        });
    });
    ...
    describe('deleteExcept', () => {
        it('should delete only supporters not present in keep list', async () => {
            const getByPlayerSpy = vi
                .spyOn(clubSupporterService, 'getByPlayer')
            ...
            const deleteSpy = vi
                .spyOn(clubSupporterService, 'delete')
                .mockResolvedValue();
    ...
    describe('deleteAll', () => {
        it('should delete all ClubSupporters', async () => {
            (prisma.clubSupporter.deleteMany as Mock).mockResolvedValueOnce({
                count: 100,
            });

...
import {
    ...
    invalidCountrySupporter,
} from '@/tests/mocks/data/countrySupporter';
...
describe('countrySupporterService', () => {
    ...
    describe('getAllWithCountry', () => {
        it('should return all country-supporter rows with country data', async () => {
            const withCountry = [
                { ...defaultCountrySupporter, country: defaultCountry },
            ];
            (prisma.countrySupporter.findMany as Mock).mockResolvedValueOnce(
                withCountry,
            );
    ...
    describe('getAll', () => {
        it('should return all CountrySupporters', async () => {
            const fixture = [
                defaultCountrySupporter,
                { ...defaultCountrySupporter, playerId: 2 },
            ];
            (prisma.countrySupporter.findMany as Mock).mockResolvedValueOnce(
                fixture,
            );
    ...
    describe('create', () => {
        it('should create a CountrySupporter', async () => {
            (prisma.countrySupporter.create as Mock).mockResolvedValueOnce(
                defaultCountrySupporter,
            );
            const result = await countrySupporterService.create(
                defaultCountrySupporter,
            );
        ...
        it('should refuse to create a CountrySupporter with invalid data', async () => {
            await expect(
                countrySupporterService.create({
                    ...defaultCountrySupporter,
                    playerId: -1,
                }),
            ).rejects.toThrow();
            await expect(
                countrySupporterService.create(invalidCountrySupporter),
            ).rejects.toThrow();
        });

        it('should refuse to create a CountrySupporter that has the same player ID and country FIFA code as an existing one', async () => {
            (prisma.countrySupporter.create as Mock).mockRejectedValueOnce(
                new Error('CountrySupporter already exists'),
            );
            await expect(
                countrySupporterService.create({
                    ...defaultCountrySupporter,
    ...
    describe('upsert', () => {
        it('should create a CountrySupporter where the combination of player ID and country FIFA code did not exist', async () => {
            (prisma.countrySupporter.upsert as Mock).mockResolvedValueOnce(
                defaultCountrySupporter,
            );
            ...
            expect(prisma.countrySupporter.upsert).toHaveBeenCalledWith({
                where: {
                    playerId_countryFIFACode: {
                        playerId: defaultCountrySupporter.playerId,
                        countryFIFACode:
                            defaultCountrySupporter.countryFIFACode,
                    },
                },
                create: {
                    playerId: defaultCountrySupporter.playerId,
    ...
    describe('upsertAll', () => {
        it('should upsert each country FIFA code for a player', async () => {
            const upsertSpy = vi
                .spyOn(countrySupporterService, 'upsert')
                .mockResolvedValue({
                    ...defaultCountrySupporter,
                    playerId: 7,
                    countryFIFACode: 'ENG',
    ...
    describe('delete', () => {
        ...
        it('should silently return when asked to delete a CountrySupporter that does not exist', async () => {
            ...
            (prisma.countrySupporter.delete as Mock).mockRejectedValueOnce(
                notFoundError,
            );
        ...
        it('should rethrow delete errors that are not P2025', async () => {
            ...
            await expect(
                countrySupporterService.delete(7, 'ENG'),
    ...
    describe('deleteExcept', () => {
        it('should delete only supporters not present in keep list', async () => {
            const getByPlayerSpy = vi
                .spyOn(countrySupporterService, 'getByPlayer')
            ...
            const deleteSpy = vi
                .spyOn(countrySupporterService, 'delete')
                .mockResolvedValue();
    ...
    describe('deleteAll', () => {
        it('should delete all CountrySupporters', async () => {
            (prisma.countrySupporter.deleteMany as Mock).mockResolvedValueOnce({
                count: 100,
            });
